...
| MB-7 | Visual identity swap — `@theme` tokens, fonts, CSP | ✅ | Tokens flipped: paper `#f5f2ec`, ink `#0f0e0d`, ink-3 `#6b6660` (AA 4.89:1 on paper), coral rust + teal moss + violet plum + amber ochre. Fonts: DM Serif Display + DM Mono. CSS chunk -15% (39.99 → 33.94 KB). Existing CSP already allowed Google Fonts. |
| MB-8 | Rebuild HeroA in Mosaic Byte aesthetic | ✅ | Split editorial layout. Subcomponent `MosaicTileGrid` (8×6, palette-cycling, `setInterval` 2.2s, gated by `usePrefersReducedMotion`). Coral primary CTA, ghost secondary, reassure bullets, location footnote. Bottom-right availability badge wired to `COPY.status`. LCP `<h1>` unanimated. |
| MB-9 | Adapt remaining components to light editorial theme | ✅ | All other components already consume `--color-*` tokens and adapted automatically. Tokenized hardcoded `#0A0A0A` in `Button.tsx` / `PricingTier.tsx` / `HeroA.tsx` to `var(--color-paper)` (the dark-theme paper hex was wrong intent on coral). MobileDrawer's two semi-transparent dark overlays kept as-is (they read identically on light theme). |
| MB-10 | Tests for rebranded structure | ✅ | Navbar + MobileDrawer tests updated for "Start a project" CTA + new nav order. `usePrefersReducedMotion` hardened against missing matchMedia so HeroA test runs cleanly in jsdom. 58/58. |
| MB-11 | Verify — tsc / lint / vitest / build | ✅ | tsc 0, lint 0, vitest 58/58, vite build ok. |
| MB-12 | Live browser verify in dev server | ✅ | Verified at http://localhost:5173/: title "Mosaic Byte — Digital Studio", body bg `#f5f2ec`, DM Mono/Serif Display, wordmark "Mosaic Byte", "Start a project" CTA, hero tile grid animating, availability badge live. |
| MB-13 | Create new `mosaicbyte` GitHub repo + push + collaborator + archive prototype | ✅ (mostly) | Repo live at `Outtsett/mosaicbyte` (private), 12 commits pushed to `main` via `gh repo create --source . --push`. Jesenia invited as collaborator with `push` permission. Pending: Jesenia accepts the invitation; Tyler pushes the local README rewrite (`b368dfa`); Jesenia archives `lundeej/mosaicbyte` (static prototype). |
| MB-14 | Honesty pass on credentials — strip tool/stack name-drops | ✅ | Per Tyler's directive (tools/builds shouldn't be mentioned without shippable examples). Removed every Webflow/Framer/Next.js/Astro/Sanity/Contentful/Figma/PostHog/Plausible/Hotjar/Lighthouse reference from `copy.ts`. `COPY.stack` block deleted; `StackBadges` + `Card` components deleted (both unused). Hero reassure, services scopes, process step, FAQ "stack" answer, about credentials all rewritten stack-agnostically. The site now sells the practice (stack-agnostic, you own the source), not the SKU list. 58/58 tests pass. |
| MB-15 | Light/dark/system theme toggle | ✅ | Three-state cycling toggle (Sun → Moon → Monitor) wired into `Navbar` + `MobileDrawer`. Defaults to OS preference; explicit choice persists to `localStorage('mosaic-theme')`. `src/lib/theme.ts` `useTheme` hook uses `useSyncExternalStore` for live OS-preference tracking (avoids `react-hooks/set-state-in-effect`). FOUC prevented by inline pre-paint script in `index.html`. Dark tokens live in `:root[data-theme="dark"]` + `@media (prefers-color-scheme: dark)`; only paper/ink axes flip, brand accents constant. Turnstile `theme:'auto'`. `Layout`'s per-route `theme` prop removed (global toggle drives it). Browser-verified system → light → dark → system cycle with localStorage persistence. 58/58 tests. |
