...
- Brand: every visible string now reads "Mosaic Byte" (driven by `src/content/copy.ts`). Domain default `mosaicbyte.design` (was `lundeen-studio.vercel.app`) in `src/lib/seo-data.ts`.
...
- Fonts: Geist + Geist Mono → DM Serif Display + DM Mono. `index.html` preconnect + stylesheet rewritten. Existing CSP allowlist already covered fonts.googleapis/gstatic
...
- **Destructive actions need explicit ask.** `git reset --hard`, `rm -rf`, dropping a table, force-pushing, deleting branches, pause and confirm with Tyler.
...
P0 launch-blockers fixed (2026-05-25): Turnstile explicit render — the form was 403ing on every submit because `api.js` was never loaded and a lazy route can't use implicit auto-scan; `--color-ink-3` darkened to WCAG AA (was 3.89:1); `/privacy` route + policy added (dead `/terms` link removed); 
...
### GitHub Pages routing fix (2026-07-28) — the mirror 404'd on every URL

Root cause and fix are documented in full in `docs/github-pages-research.md`. Summary: the workflow above (`.github/workflows/pages.yml`) and the `dist/index.html` → `dist/404.html` copy were both already correct — GitHub Pages serves 404.html's content AT the originally-requested URL with no redirect, so `location.pathname` stays correct (e.g. `/MosaicByte/about`). The actual bug was that **wouter (the client router) was never told about the `/MosaicByte` subpath**, so route matching failed against every `<Route path="...">` even though the pathname was correct, and separately every plain `<a href="/...">` bypassed subpath-awareness entirely (only wouter's own `<Link>` accounts for it).

Fixed:

- **`src/main.tsx`**: wraps the tree in wouter's `<Router base={ROUTER_BASE}>`, where `ROUTER_BASE = import.meta.env.BASE_URL.replace(/\/$/, '')`. **The trailing-slash strip is load-bearing, not cosmetic** — Vite's `BASE_URL` always ends in `/` (`/MosaicByte/` on Pages, `/` on Vercel), but wouter's `base` does a plain string-prefix strip against `location.pathname` with no trailing-slash special-casing of its own. Passing `/MosaicByte/` verbatim strips the boundary slash along with the prefix, turning `/MosaicByte/about` into the relative path `about` (no leading slash) instead of `/about` — and `regexparam` compiles every `<Route path="/about">` as `^\/about\/?$`, which requires that leading slash. Verified directly against wouter's `relativePath()` + `regexparam` (`node -e` repro in this session): with the un-stripped base, `/about` fails to match (only `/` accidentally still works); with the stripped base (`/MosaicByte`, no trailing slash), `/about` and `/` both match correctly, and the Vercel build is unaffected (`'/'.replace(/\/$/, '') === ''`, a no-op). Router wraps `<ErrorBoundary>` (not the reverse) so the error-fallback UI — which now also uses wouter's `<Link>` for its "Back to home" link — always renders inside a valid routing context, even when the crash happened inside `<App />` itself.
- **`src/lib/utils.ts`**: added `withBasePath(href)` — prefixes an internal href that can't go through `<Link>` (hash-scroll anchors like `/#process`, or the static `/sitemap.xml` link) with `import.meta.env.BASE_URL`, stripping the href's own leading slash first so the concatenation doesn't produce `//`.
- **Converted every internal navigation href** across `src/components/hero/HeroA.tsx`, `src/components/ui/PricingTier.tsx`, `src/components/layout/Navbar.tsx`, `src/components/layout/MobileDrawer.tsx`, `src/components/layout/Footer.tsx`, `src/components/ErrorBoundary.tsx`, `src/routes/{Home,About,Contact,Work,NotFound}.tsx`. Pattern used throughout: `href.includes('#') ? <a href={withBasePath(href)}>` for hash anchors, else `<Link href={href}>` for real routes (mirrors the pattern Navbar/MobileDrawer already used for their nav-item lists — extended to every other CTA/link in the app). Found during the required post-edit re-grep (not in the original fix list): `Footer.tsx`'s hash/static-asset branch and its banner CTA, plus `src/routes/Work.tsx` and `src/routes/NotFound.tsx` (the 404 page's own "Back to homepage" link) were also unprefixed plain `<a>` tags — all fixed the same way. `NotFound.tsx` matters especially: it's the exact page GitHub Pages serves as `404.html`, so its own home link being broken would have been the worst version of this bug.
- **`scripts/generate-sitemap.mjs`**: now also generates `dist/robots.txt` (was a static `public/robots.txt`, deleted) using the same `VITE_SITE_URL` env var already read for `sitemap.xml`, so the `Sitemap:` line always matches whichever origin actually built the site (`https://lundeen-labs.github.io/MosaicByte/sitemap.xml` on Pages, `https://mosaicbyte.design/sitemap.xml` on Vercel) instead of always hardcoding the Vercel URL.
- **`public/.nojekyll`** (new, empty): defensive per the research doc — Jekyll doesn't run in this pipeline today (no legacy branch-source publishing, no `jekyll-build-pages` step), so this is zero-cost insurance against a future underscore-prefixed asset folder silently vanishing if the publishing source ever changes.

Verified 2026-07-28: same `tsc`/`lint`/`vitest` (58/58) green after this fix. Pages-mode build's `dist/robots.txt` correctly reads `Sitemap: https://lundeen-labs.github.io/MosaicByte/sitemap.xml`; plain Vercel-mode build's `dist/robots.txt` correctly reads `Sitemap: https://mosaicbyte.design/sitemap.xml`. `dist/.nojekyll` present in the Pages build output. Re-grepped all of `src/**/*.tsx` post-edit for `href="/`/`href={` patterns — zero unconverted internal hrefs remain (only `<Link>`/`withBasePath()` usages, external `mailto:`/`https://` links, and the bare in-page `#main` skip-link, which needs no prefix since it has no leading slash).
