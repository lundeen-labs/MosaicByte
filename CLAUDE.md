...
The site now builds and deploys to a **static GitHub Pages mirror** at `https://lundeen-labs.github.io/MosaicByte/`, additive alongside the primary Vercel deploy. Vercel remains production; Pages is a secondary static mirror. Nothing about the Vercel build path changed — with no env vars set, `npm run build` still emits root-relative assets (`/assets/...`, `/favicon.svg`), verified 2026-07-28.
...
- **`scripts/generate-sitemap.mjs`**: now also generates `dist/robots.txt` (was a static `public/robots.txt`, deleted) using the same `VITE_SITE_URL` env var already read for `sitemap.xml`, so the `Sitemap:` line always matches whichever origin actually built the site (`https://lundeen-labs.github.io/MosaicByte/sitemap.xml` on Pages, `https://mosaicbyte.vercel.app/sitemap.xml` on Vercel) instead of always hardcoding the Vercel URL.
